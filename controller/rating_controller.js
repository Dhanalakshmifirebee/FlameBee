  ...
  try {
    ratingController.Rating.create(req.body, (err, data) => {
      if (err) {
        ...
        ratingController.Rating.countDocuments({ foodId: data.foodId },(err, num) => {
            if (err) {throw err;
            } else {
              console.log(num);
              const noOfPersons = num;
              ratingController.Rating.find( { foodId: data.foodId },{ rating: 1, _id: 0 },(err, datas) => {
                  if (err) {
                    throw err;
                  } else {
                    console.log(datas);
                    ...
                    console.log(average);
                    restarauntControll.addAvaliableFood.findOneAndUpdate({ _id: req.body.foodId },{ $set: { rating: average } },
                      { new: true },
                      (err, data) => {
                        if (err) {
                          throw err;
